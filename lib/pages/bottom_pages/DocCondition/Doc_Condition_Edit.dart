...
class EditDocConditionPage extends StatefulWidget {
  final String id;
  const EditDocConditionPage({Key? key, required this.id}) : super(key: key);

  @override
  State<EditDocConditionPage> createState() => _EditDocConditionPageState();
}

class _EditDocConditionPageState extends State<EditDocConditionPage> {
  bool loading = false;
  ...
  Future<Null> listemp()async{
    String url = pathurl.listempCondition+'?pos_id=${dataID[0]['emp_created']['pos_id']}'; /*${dataID[0]['emp_created']['pos_id']}*/
    final response = await get(
        Uri.parse(url),
        headers: {"Authorization": "Bearer $token"}
    );
  ...
  Future<Null> getdata() async {
    try{
      ...
      String url = pathurl.getDTDocConditionID+widget.id;
  ...
  Future<Null> senddata(data) async{
    String url = pathurl.updateDTDocConditionID+widget.id;
  ...
  void initState() {
    ...
    Future.delayed(const Duration(seconds: 1),(){
      ...
      selectedCondition = dataID[0]['cc_title'];
      _detialother.text = dataID[0]['cc_other'];
      _detial.text = dataID[0]['cc_details'];
      _dateActive.text = dataID[0]['cc_date'];
      selectedPO[0] = dataID[0]['cc_emp']['id'];
      print("selectedPO = $selectedPO");
      ListempID.add(dataID[0]['cc_emp']['id']);
      print("ListempID = $ListempID");
      textEditingControllers[0].text = "${dataID[0]['cc_emp']['name'].split(" ")[1]} ${dataID[0]['cc_emp']['name'].split(" ")[2]}";
      wlPos = dataID[0]['cc_emp']['pos_id'];
      print("wlPos = $wlPos");
      _detial.addListener(()=> setState(() {}));

      print("posID = ${dataID[0]['emp_created']['pos_id']}");
  ...
  Widget build(BuildContext context) {
    return Scaffold(
        ...
        appBar: AppBar(
          title: const Text("แก้ไขใบเปลี่ยนสภาพพนักงาน"),
          centerTitle: true,
          elevation: 0,
                                        ...
                                        onPressed: selectedCondition != null &&
                                          ...
                                          if (form!.validate()) {
                                            ...
                                            var jsdt = {
                                              ...
                                              "cc_date": activeDate == null
                                                        ? DateFormat("yyyy-MM-dd").format(DateFormat("dd/MM/yyyy").parse(_dateActive.text))
              ...
              children: [
                ...
                DropdownButtonFormField<String>(
                  ...
                  items: ListPos.map((valueItem) {
                    return DropdownMenuItem<String>(
                        value: valueItem['id'].toString(),child: Text(valueItem['pos_name'].toString())
                    );
                  }).toList(),
