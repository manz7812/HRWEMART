...
import 'Doc_ChangKa_Detail.dart';

class insertDocChangeKa extends StatefulWidget {
  const insertDocChangeKa({Key? key}) : super(key: key);

  @override
  State<insertDocChangeKa> createState() => _insertDocChangeKaState();
}

class _insertDocChangeKaState extends State<insertDocChangeKa> {
  ...
  Future<Null> getlistPos() async{
    String UrlPos = pathurl.listPosKa;
  ...
  Future<Null> getdata() async {
    try{
      ...
      String url = pathurl.getDTK+"?status=$selectedStatus&year=${selectedYear}&month=$selectedMonth";
  ...
  Future<Null> searchdata(year,month,status,posId) async{
    try{
      ...
      final String url = pathurl.getDTK+'?pos_id=$posId&status=$status&year=$year&month=$month';
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: loading ?Column(
        children: [
          ButtonInsert().show_ButtonInsert_KA(context,onGoBack),
          Expanded(
  ...
  Widget getdataKa(item){
    ...
    return InkWell(
      onTap: (){
        // Navigator.push(context, MaterialPageRoute(builder: (context) => DetailDocChangeKaPage(id:id))).then(onGoBack);
        Navigator.of(context).push(
            PageTransition(
              child: DetailDocChangeKaPage(id: id),
  ...
  Widget test(){
    return InkWell(
      onTap: (){
        // Navigator.push(context, MaterialPageRoute(builder: (context) => const DetailDocChangeKaPage()));
      },
      splashColor: Colors.transparent,
      highlightColor: Colors.white38,
      borderRadius: BorderRadius.circular(5),
                ...
                child: Column(
                  children: [
                    ...
                    Container(
                      ...
                      padding: const EdgeInsets.only(top: 15),
                      child: const Text(
                        "ขอวันที่ 29/03/2022 เปลี่ยนเป็น WC0002:06:30-10:30 - 11:30-15:30",
                        style: TextStyle(
                          color: Colors.black,
                          fontWeight: FontWeight.w500,
                          fontSize: 14,
                    ...
                    Container(
                      ...
                      padding: const EdgeInsets.only(top: 15,bottom: 15),
                      child: const Text(
                        "หมายเหตุ : แทนพนักงานB เนื่องจากBลาพักร้อน",
                        style: TextStyle(
                          color: Colors.black,
                          fontWeight: FontWeight.w500,
                          fontSize: 14,
