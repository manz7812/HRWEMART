...
import 'Detail_Data_DocAddTime.dart';

class ListDataApproveDocAddTimePage extends StatefulWidget {
  const ListDataApproveDocAddTimePage({Key? key}) : super(key: key);

  @override
  State<ListDataApproveDocAddTimePage> createState() => _ListDataApproveDocAddTimePageState();
}

class _ListDataApproveDocAddTimePageState extends State<ListDataApproveDocAddTimePage> {
  ...
  Future<Null> getdata() async{
    try{
      ...
      final String url = pathurl.getDTApDocAddTime+'?status=$selectedStatus&year=$selectedYear&month=$selectedMonth';
      final response = await get(
          Uri.parse(url),
          headers: {"Authorization": "Bearer $token"}
      );
  ...
  Future<Null> searchdata(year,month,status,emp) async{
    try{
      ...
      final String url = pathurl.getDTApDocAddTime+"?status=${status}&year=${year}&month=${month}&emp=${emp}";
      final response = await get(
          Uri.parse(url),
          headers: {"Authorization": "Bearer $token"}
      );
  ...
  Widget build(BuildContext context) {
    return Scaffold(
        ...
        appBar: AppBar(
          title: const Text("อนุมัติเอกสารขอเพิ่มเวลา"),
          centerTitle: true,
          elevation: 0,
  ...
  Widget getCard(item) {
    var fullName = item['at_emp']['name'].toString().split(' ');
    var firstName = fullName;
    var nickName = item['at_emp']['nick_name'];
    var datecreated = item['at_date_created'];
    var detail = item['at_details'];
    var remark = item['at_remark'];
    var status = item['at_status'];
    var IDdoc = item['at_id'];
    var appStatus = item['at_approve'];
    var Sdate = item['at_date'];
    bool approve = false;
                  ...
                  child: Column(
                    children: [
                      ...
                      Container(
                        ...
                        alignment: Alignment.centerLeft,
                        child: Text(
                          "${firstName[1]} ${firstName[2]}($nickName) ยื่นขอ เพิ่มเวลา ",
                          style: TextStyle(
                    ...
                    ElevatedButton(///รายละเอียด
                        ...
                        onPressed: (){
                          Navigator.of(context).push(
                              PageTransition(
                                child: DetailApproveDocAddTime(id: IDdoc),
