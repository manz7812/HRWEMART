...
import 'Detail_DocHuman_Approve.dart';

class DetailDocHumanApprovePage extends StatefulWidget {
  final Function? onGoBack;
  DetailDocHumanApprovePage({Key? key ,this.onGoBack}) : super(key: key);

  @override
  State<DetailDocHumanApprovePage> createState() => _DetailDocHumanApprovePageState();
}

class _DetailDocHumanApprovePageState extends State<DetailDocHumanApprovePage> {
  ...
  Future<Null> getlistempName() async{
    String UrlPos = pathurl.listEmpName;
  ...
  Future<Null> getdata() async{
    try{
      ...
      final String url = pathurl.urldataApprove+'?status=$selectedStatus&year=$selectedYear&month=$selectedMonth&emp=$selectedEmployeename';
      final response = await get(
          Uri.parse(url),
          headers: {"Authorization": "Bearer $token"}
      );
  ...
  Future<Null> searchdata(year,month,status,emp) async{
    try{
      ...
      final String url = pathurl.urldataApprove+'?status=$status&year=$year&month=$month&emp=$emp';
      final response = await get(
          Uri.parse(url),
          headers: {"Authorization": "Bearer $token"}
      );
  ...
  Widget build(BuildContext context) {
    return Scaffold(
        ...
        appBar: AppBar(
          title: const Text("อนุมัติเอกสารขอกำลังพล"),
          centerTitle: true,
          elevation: 0,
  ...
  Widget getCard(item) {
    var fullName = item['req_emp']['name'].toString().split(' ');
    var firstName = fullName;
    var nickName = item['req_emp']['nick_name'];
    var pos = item['req_pos']['name'];
    var datecreated = item['req_date_created'];
    var note = item['req_remark'];
    var status = item['req_status'];
    var IDdoc = item['req_id'];
    var appStatus = item['req_approve'];
    bool approve = false;
                  ...
                  child: Column(
                    children: [
                      ...
                      Container(
                        ...
                        alignment: Alignment.centerLeft,
                        child: Text(
                          "${firstName[1]} ${firstName[2]}($nickName) ยื่นขอกำลังพล\n''${pos}'' ",
                          style: TextStyle(
                    ...
                    ElevatedButton(///รายละเอียด
                        ...
                        onPressed: (){
                          ...
                          Navigator.of(context).push(
                              PageTransition(
                                  child: DetailApproveDocHuman(id: IDdoc),
