...
import 'Detail_DocResign_Approve.dart';


class ListDataApproveDocResignPage extends StatefulWidget {
  const ListDataApproveDocResignPage({Key? key}) : super(key: key);

  @override
  State<ListDataApproveDocResignPage> createState() => _ListDataApproveDocResignPageState();
}

class _ListDataApproveDocResignPageState extends State<ListDataApproveDocResignPage> {
  ...
  Future<Null> getdata() async{
    try{
      ...
      final String url = pathurl.getDTAppDocResign+'?status=$selectedStatus&year=$selectedYear&month=$selectedMonth&emp=$selectedEmployeename';
      final response = await get(
          Uri.parse(url),
          headers: {"Authorization": "Bearer $token"}
      );
  ...
  Future<Null> searchdata(year,month,status,emp) async{
    try{
      ...
      final String url = pathurl.getDTAppDocResign+"?status=${status}&year=${year}&month=${month}&emp=${emp}";
      final response = await get(
          Uri.parse(url),
          headers: {"Authorization": "Bearer $token"}
      );
  ...
  Widget build(BuildContext context) {
    return Scaffold(
        ...
        appBar: AppBar(
          title: const Text("อนุมัติเอกสารขอลาออก"),
          centerTitle: true,
          elevation: 0,
  ...
  Widget getCard(item) {
    var fullName = item['re_emp']['name'].toString().split(' ');
    var firstName = fullName;
    var nickName = item['re_emp']['nick_name'];
    var detail = item['re_details'];
    var status = item['re_status'];
    var IDdoc = item['re_id'];
    var appStatus = item['re_approve'];
    var Sdate = item['re_date'];
    bool approve = false;
                  ...
                  child: Column(
                    children: [
                      ...
                      Container(
                        ...
                        alignment: Alignment.centerLeft,
                        child: Text(
                          "${firstName[1]} ${firstName[2]}($nickName) ยื่นขอ ลาออก ",
                          style: TextStyle(
                    ...
                    ElevatedButton(///รายละเอียด
                        ...
                        onPressed: (){
                          Navigator.of(context).push(
                              PageTransition(
                                child: DetailApproveDocResign(id: IDdoc),
