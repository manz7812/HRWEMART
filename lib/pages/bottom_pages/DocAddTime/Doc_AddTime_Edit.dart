...
class EditDocAddTimePage extends StatefulWidget {
  final String id;
  const EditDocAddTimePage({Key? key,required this.id}) : super(key: key);

  @override
  State<EditDocAddTimePage> createState() => _EditDocAddTimePageState();
}

class _EditDocAddTimePageState extends State<EditDocAddTimePage> {
  ...
  Future<Null> senddata() async {
    try{
      ...
      FormData formData = FormData.fromMap({
        "at_img": fileChange == true ? await MultipartFile.fromFile(
          file!.path,
          filename: nameIMG,
          contentType: MediaType('image','jpg'),
        ) : null,
        "old_img": dataAdTime[0]['at_img'],
      ...
      String url = pathurl.updateDTHedPonEditID+widget.id;
      ...
      if(e.response?.statusCode == 401){
        ...
        setState(() {
          AwesomeDialog(
            ...
            // dialogBackgroundColor: Colors.deepPurpleAccent,
            title: 'บันทึกข้อมูลไม่สำเร็จ',
            desc: 'คุณเคยทำรายการแล้ว กรุณาทำรายใหม่อีกครั้ง',
            btnOkText: "ตกลง",
            btnCancelText: "ยกเลิก",
            btnOkOnPress: () {
              // Navigator.pop(context);
  ...
  void initState() {
    ...
    Future.delayed(const Duration(seconds: 1),() async{
      setState(() {
        loading = true;
        var dt = DateFormat("yyyy-MM-dd").parse(dataAdTime[0]['at_date']);
        var listdt = dt.toString().split(" ")[0].split("-");
        print("$listdt");
        _dateAddTimeText.text = "${listdt[2]}/${listdt[1]}/${listdt[0]}";
        checkStatusday(dt.toString().split(" ")[0]);
        selectedInOut = dataAdTime[0]['at_inout'];
        _time.text = dataAdTime[0]['at_time'];
        selectedHedpon = dataAdTime[0]['at_remark'];
      });
      await Future.delayed(const Duration(milliseconds: 50));
      ListHedpon.forEach((element) {
        if(element['name'] == selectedHedpon){
          checkIMG = element['img'];
          print("checkIMG = $checkIMG");
        }
      });
      _detailText.text = dataAdTime[0]['at_details'];
      print("file = ${file}");
    });
  ...
  Widget build(BuildContext context) {
    return Scaffold(
        ...
        appBar: AppBar(
          title: const Text("แก้ไขขอเพิ่มเวลา"),
          // leading: IconButton(
          //   onPressed: () {
            ...
            Stack(
              children: <Widget>[
                ClipRRect(
                    ...
                    clipBehavior: Clip.antiAlias,
                    child: Image.network(dataAdTime[0]['at_img'],fit: BoxFit.cover)),
                Positioned(
