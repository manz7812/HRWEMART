...
import 'Doc_Salary_Certificate_Detail.dart';

class insertDocSalaryCertificate extends StatefulWidget {
  const insertDocSalaryCertificate({Key? key}) : super(key: key);

  @override
  State<insertDocSalaryCertificate> createState() => _insertDocSalaryCertificateState();
}

class _insertDocSalaryCertificateState extends State<insertDocSalaryCertificate> {
  ...
  List ListStatus = [
    'ทั้งหมด', 'อนุมัติ', 'ไม่อนุมัติ','ตีกลับ','ยกเลิก','ส่งเอกสาร','สำเร็จ',
  ];
  ...
  List dataDocSalaryCer = [];
  Future<Null> getdata() async {
    try{
      setState(() {
        loading = false;
      });
      String url = pathurl.getDTDocSalaryCer+"?status=$selectedStatus&year=$selectedYear&month=$selectedMonth";
  ...
  Future<Null> setStatus(id,status,remark) async{
    try{
      var url = pathurl.setStatusDTDocSalaryCerID+id;
  ...
  Future<Null> searchdata(year,month,status) async{
    try{
      ...
      final String url = pathurl.getDTDocSalaryCer+'?status=$status&year=$year&month=$month';
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        title: const Text("ขอหนังสือรับรองเงินเดือน"),
        centerTitle: true,
        elevation: 0,
          ...
          children: [
            SingleChildScrollView(
              child: Column(
                ...
                children: [
                  ButtonInsert().show_ButtonInsert_Request_salary_certificate(context,onGoBack),
                ],
              ),
            ),
            Expanded(
                child: SingleChildScrollView(
                  child: Column(
                    children: [
                      ...
                      dataDocSalaryCer.isEmpty ? Nulldata() :
                      ListView.builder(
                        itemCount: dataDocSalaryCer.length,
                        scrollDirection: Axis.vertical,
                        shrinkWrap: true,
                        physics: ScrollPhysics(),
                        itemBuilder: (context , i){
                          return getCard(dataDocSalaryCer[i]);
                        },
                      ),
                    ],
                  ),
  ...
  Widget getCard(item){
    var IDdoc = item['sc_id'];
    var dtCreate = item['sc_date_created'];
    var fullName = item['sc_emp']['name'];
    var date= item['sc_date'];
    var reason = item['sc_reason'];
    var detail = item['sc_details'];
    var status = item['sc_status'];
                      ...
                      child: TextButton(
                          ...
                          onPressed: (){
                            ...
                            Navigator.of(context).push(
                                PageTransition(
                                  child: DetailDocSalaryCertificatePage(id: IDdoc,),
