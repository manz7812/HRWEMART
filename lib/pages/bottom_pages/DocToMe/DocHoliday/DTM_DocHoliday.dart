...
import 'DTM_DocHoliday_Detail.dart';

class DTMDocHolidayPage extends StatefulWidget {
  const DTMDocHolidayPage({Key? key}) : super(key: key);

  @override
  State<DTMDocHolidayPage> createState() => _DTMDocHolidayPageState();
}

class _DTMDocHolidayPageState extends State<DTMDocHolidayPage> {
  ...
  Future<Null> getdata() async {
    try{
      ...
      String url = pathurl.dtmHolyday+'?status=$selectedStatus&year=$selectedYear&month=$selectedMonth';;
  ...
  Future<Null> searchdata(year,month,status) async{
    try{
      ...
      final String url = pathurl.dtmHolyday+'?status=$status&year=$year&month=$month';
      final response = await get(
          Uri.parse(url),
          headers: {"Authorization": "Bearer $token"}
      );
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        title: const Text("เอกสารเปลี่ยนวันหยุด"),
        // leading: IconButton(
        //   onPressed: () {
    ...
    return InkWell(
      onTap: (){
        ...
        Navigator.of(context).push(
            PageTransition(
              child: DTMDocHolidayDetailPage(id: id),
