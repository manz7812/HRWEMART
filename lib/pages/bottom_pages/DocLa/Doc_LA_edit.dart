...
class EditDocLa extends StatefulWidget {
  final String id;
  EditDocLa({Key? key,required this.id}) : super(key: key);

  @override
  State<EditDocLa> createState() => _EditDocLaState();
}

class _EditDocLaState extends State<EditDocLa> {
  ...
  Future<Null> getdata() async {
    SharedPreferences preferences = await SharedPreferences.getInstance();
    token = preferences.getString("token")??"";
    String url = pathurl.editLa+widget.id;
  ...
  Future<DateTime?> pickStartDate(BuildContext context) async {
    ...
    var datecreated = DateFormat('dd/MM/yyyy').parse(dataLa[0]['rl_date_created']);
    print(datecreated.day);


    print(BANum);

    if(BA == "A"){ ///ย้อนหลัง
      initialDate = DateTime(datecreated.year, datecreated.month, datecreated.day -int.parse(BANum));
      firstDate = DateTime(datecreated.year, datecreated.month, datecreated.day -int.parse(BANum));
      lastDate = DateTime(datecreated.year, datecreated.month, datecreated.day -int.parse(BANum));///วัน ปจบ
      ...
      WHD.forEach((element) {
        if(element == dt){
          initialDate = DateTime(datecreated.year, datecreated.month, datecreated.day -int.parse(BANum)-int.parse(BANum));
          firstDate = DateTime(datecreated.year, datecreated.month, datecreated.day -int.parse(BANum)-int.parse(BANum));
          lastDate = DateTime(datecreated.year, datecreated.month, datecreated.day -int.parse(BANum)-int.parse(BANum));
          print(initialDate);
        }
      });

    }else if(BA == "B"){////ล่วงหน้า
      initialDate = DateTime(datecreated.year, datecreated.month, datecreated.day +int.parse(BANum)); ///วัน ปจบ
      firstDate = DateTime(datecreated.year, datecreated.month, datecreated.day+int.parse(BANum));
      lastDate = DateTime(datecreated.year, datecreated.month+int.parse(BANum), datecreated.day +int.parse(BANum));
    }else{///E
      initialDate = DateTime(datecreated.year, datecreated.month, datecreated.day); ///วัน ปจบ
      firstDate = DateTime(datecreated.year, datecreated.month, datecreated.day-1);
      lastDate = DateTime(datecreated.year, datecreated.month +1, datecreated.day);
    }
  ...
  Future<DateTime?> pickEndDate(BuildContext context) async {
    ...
    if (newdate == null){
      ...
      var DOW = List.generate(diff, (index) => index).map((value) =>
          DateFormat(DateFormat.WEEKDAY, 'th').format(startdateTime!.add(Duration(days: value)))).toList();
      var DOY = List.generate(diff, (index) => index).map((value) =>
          DateFormat("dd/MM/yyyy").format(startdateTime!.add(Duration(days: value)))).toList();
      print(DOW);
      print(DOY);
      int i = 0;
      print(proviso);
  ...
  Future<Null> senddata() async{
    var startdate = _Startdatetime.text;
    var enddate = _EnddatetimeFull.text;

    var startdate2 = DateFormat('dd/MM/yyyy HH:mm').parse(_Startdatetime.text);
    var enddate2 = DateFormat('dd/MM/yyyy HH:mm').parse(_EnddatetimeFull.text);


    if(startdateTime !=null && enddateTime != null){
      switch(halfID){
        case 1:
          startdate = DateFormat('yyyy-MM-dd').format(startdateTime!) +" $timeStart";
    ...
    if(dateArray.isEmpty){
      var numday = int.parse(day.text);
      var DOW = List.generate(numday, (index) => index).map((value) =>
          DateFormat(DateFormat.WEEKDAY, 'th').format(startdate2.add(Duration(days: value)))).toList();
      var DOY = List.generate(numday, (index) => index).map((value) =>
          DateFormat("dd/MM/yyyy").format(startdate2.add(Duration(days: value)))).toList();
      int i = 0;
      if(proviso == "true"){
        i=0;
      }else{
    ...
    FormData formData = FormData.fromMap({
      'rl_img': fileChange == true ? await MultipartFile.fromFile(
        file!.path,
        filename: nameIMG,
        contentType: MediaType('image','jpg'),
      ) : null,
      'old_img': dataLa[0]['rl_img'],
      'leave_type': selectedListLa,
      'leave_format': halfID,
      'rl_date_start': startdateTime !=null ? startdate : startdate2,
      'rl_date_end': enddateTime != null ? enddate : enddate2,
  ...
  void initState() {
    ...
    Future.delayed(const Duration(seconds: 1),(){
      setState(() {
        selectedListLa = dataLa[0]['leave_type'].toString();
        setFile(dataLa[0]['leave_type'].toString());
        halfID = dataLa[0]['leave_format'];
        selectHalfDay = HalfDay[dataLa[0]['leave_format']-1].toString();
            // dataLa[0]['leave_format'].toString();
        _Startdatetime.text = dataLa[0]['rl_date_start'];
        _EnddatetimeFull.text = dataLa[0]['rl_date_end'];
        day.text = dataLa[0]['rl_num'].toString();
        _detial.text = dataLa[0]['rl_details'];
        loading = true;
      });
    });
    _detial.addListener(() {
                        ...
                        child: Column(
                          children: [
                            ...
                            DropdownButtonFormField<String>(
                              ...
                              //   });
                              // },
                              validator: (value) => (selectedListLa == '' || selectedListLa == null)
                                  ? ''
                                  : null,
                            ...
                            Column(
                              children: HalfDay
                                  ...
                                  value: t.toString(),
                                  onChanged: selectedListLa == null || quota == 0 || Myquota == 0 ? null : (val) =>
                                      setState(() {
                                        selectHalfDay = val!;
                                        // dataLa[0]['leave_format'] = val;
                                        _Startdatetime.clear();
                                        _EnddatetimeFull.clear();
                                        _Enddatetime.clear();
                                        _detial.clear();
                            ...
                            TextFormField(
                              ...
                              enabled: selectHalfDay != null ? true : false,
                              validator: (value) => (value!.isEmpty) ? '' : null,
                              decoration: InputDecoration(
                                // label: Text("เลือกวันที่"),
                                filled: selectHalfDay != null ? false : true,
                                fillColor: Colors.grey.shade200,
                                suffixIcon: Icon(
                                  Icons.calendar_today_outlined,
                                  color: selectHalfDay != null ? Colors.green : Colors.grey,
                                ),
                                border: OutlineInputBorder(),
                              ),
                              onTap: () async{
                                ...
                                _Startdatetime.text = DateFormat('dd/MM/yyyy').format(startdateTime!) +" $timeStart";
                                switch(halfID){
                                  case 2:
                                    _EnddatetimeFull.text = DateFormat('dd/MM/yyyy').format(startdateTime!) +" $timeEnd";
                                    break;
                                  case 3:
                                    _EnddatetimeFull.text = DateFormat('dd/MM/yyyy').format(startdateTime!) +" $timeEnd";
                                    break;
                            ...
                            ?Column(
                              children: [
                                ...
                                TextFormField(
                                  ...
                                  enabled: _Startdatetime.text.isNotEmpty  ? true : false,
                                  decoration: InputDecoration(
                                    // label: Text("เลือกวันที่"),
                                    filled: _Startdatetime.text.isNotEmpty ? false : true,
                                    fillColor: Colors.grey.shade200,
                                    suffixIcon: Icon(
                                      Icons.calendar_today_outlined,
                                      color: _Startdatetime.text.isNotEmpty ? Colors.green : Colors.grey,
                                    ),
                                    border: OutlineInputBorder(),
                                  ),
                                  onTap: () async{
                                    ...
                                    _EnddatetimeFull.text = DateFormat('dd/MM/yyyy').format(enddateTime!) +" $timeEnd";
                                  },
                                ),
                                const SizedBox(
                                  height: 20,
                                ...
                                TextFormField(
                                  ...
                                  decoration: InputDecoration(
                                    ...
                                    suffixIcon: Icon(
                                      Icons.calendar_today_outlined,
                                      color: _EnddatetimeFull.text.isNotEmpty ? Colors.green : Colors.grey,
                                    ),
                                    border: OutlineInputBorder(),
                                  ),
                            ...
                            TextFormField(
                              controller: _detial,
                              // focusNode: FocusNode(),
                              enabled: selectedListLa == null || quota == 0 || Myquota == 0 || _EnddatetimeFull.text.isEmpty ? false : true,
                              validator: (value) => (value!.isEmpty) ? '' : null,
                              maxLines: 6,
                              // maxLength: 9999,
                              decoration: InputDecoration.collapsed(
                                filled: selectedListLa == null || _EnddatetimeFull.text.isEmpty ? true : false,
                                fillColor: Colors.grey.shade200,
            ...
            Stack(
              children: <Widget>[
                ClipRRect(
                    ...
                    child: Image.network(dataLa[0]['rl_img'],fit: BoxFit.cover)),
                Positioned(
      ...
      child: Column(
        children: [
          LinearPercentIndicator(
            ...
            lineHeight: 20,
            percent: total,
            center: Text("คงเหลือ ${Myquota} / ${quota} วัน",style: TextStyle(fontWeight: FontWeight.w600,color: Colors.black87),),
