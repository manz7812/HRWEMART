...
import 'package:index/pages/bottom_pages/DocHoliday/Doc_ChangeHoliday_Detail.dart';
...
class insertDocChangeHoliday extends StatefulWidget {
  const insertDocChangeHoliday({Key? key}) : super(key: key);

  @override
  State<insertDocChangeHoliday> createState() => _insertDocChangeHolidayState();
}

class _insertDocChangeHolidayState extends State<insertDocChangeHoliday> {
  ...
  Future<Null> getlistPos() async{
    String UrlPos = pathurl.listPosHoly;
  ...
  Future<Null> getdata() async {
    try{
      ...
      String url = pathurl.serchDTH+'?pos_id=$selectedPos&status=$selectedStatus&year=$selectedYear&month=$selectedMonth';;
      final response = await get(
          Uri.parse(url),
          headers: {"Authorization": "Bearer $token"}
      );
  ...
  Future<Null> searchdata(year,month,status,posId) async{
    try{
      ...
      final String url = pathurl.serchDTH+'?pos_id=$posId&status=$status&year=$year&month=$month';
      final response = await get(
          Uri.parse(url),
          headers: {"Authorization": "Bearer $token"}
      );
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: loading ? Column(
        children: [
          ButtonInsert().show_ButtonInsert_Holiday(context,onGoBack),
          Expanded(
              child: SingleChildScrollView(
                child: Column(
                  children: [
                    ...
                    dataDocHoly.isEmpty ? Nulldata() :
                    ListView.builder(
                      itemCount: dataDocHoly.length,
                      scrollDirection: Axis.vertical,
                      shrinkWrap: true,
                      physics: ScrollPhysics(),
                      itemBuilder: (context , i){
                        return getdataHoly(dataDocHoly[i]);
                      },
                    ),
  ...
  Widget getdataHoly(item){
    ...
    return InkWell(
      onTap: (){
        // Navigator.push(context, MaterialPageRoute(builder: (context) => DetailDocChangeHolidayPage(id:id))).then(onGoBack);
        Navigator.of(context).push(
            PageTransition(
              child: DetailDocChangeHolidayPage(id: id),
  ...
  Widget test(){
    return InkWell(
      onTap: (){
        // Navigator.push(context, MaterialPageRoute(builder: (context) => const DetailDocChangeHolidayPage()));
      },
      splashColor: Colors.transparent,
      highlightColor: Colors.white38,
      borderRadius: BorderRadius.circular(5),
                ...
                child: Column(
                  children: [
                    ...
                    Container(
                      alignment: Alignment.centerLeft,
                      child: const Text(
                        "กรรณิการ์ แก้วอุดม () ยื่นขอ ''เปลี่ยนวันหยุด'' ",
                        style: TextStyle(
                          color: Colors.blueAccent,
                          fontWeight: FontWeight.bold,
                          fontSize: 14,
                    ...
                    Container(
                      ...
                      padding: const EdgeInsets.only(top: 15),
                      child: const Text(
                        "ขอวันที่ 31/03/2022 เปลี่ยนเป็น วันหยุดพนักงาน",
                        style: TextStyle(
                          color: Colors.black,
                          fontWeight: FontWeight.w500,
                          fontSize: 14,
                    ...
                    Container(
                      ...
                      padding: const EdgeInsets.only(top: 15,bottom: 15),
                      child: const Text(
                        "หมายเหตุ : สลับวันหยุดจากวันอาทิตย์ที่ 3/04/65",
                        style: TextStyle(
                          color: Colors.black,
                          fontWeight: FontWeight.w500,
                          fontSize: 14,
