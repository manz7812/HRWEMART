...
class DocWaringPage extends StatefulWidget {
  const DocWaringPage({Key? key}) : super(key: key);

  @override
  State<DocWaringPage> createState() => _DocWaringPageState();
}

class _DocWaringPageState extends State<DocWaringPage> {
  bool loading = false;
  ...
  Future<Null> listemp() async {
    String url = pathurl.listempWarning+"?pos_id=$selected2Pos";
    final response = await get(
        Uri.parse(url),
        headers: {"Authorization": "Bearer $token"}
    );
  ...
  Future<Null> senddata(data) async{
    String url = pathurl.sendDTDocWarning;
  ...
  void initState() {
    ...
    Future.delayed(const Duration(seconds: 1),(){
      ...
      _detailheader.addListener(() {
        setState(() {});
      });
      _repeatsame.addListener(() {
        setState(() {});
      });
      _otherdetail.addListener(() {
        setState(() {});
      });
                          ...
                          Column(
                            children: [
                              ...
                              TextFormField(
                                ...
                                showCursor: true,
                                readOnly: false,
                                enabled: selectedMistake != null ? true : false,
                                validator: (value) => (value!.isEmpty) ? '' : null,
                                maxLines: 6,
                                // maxLength: 9999,
                                decoration: InputDecoration(
                                  contentPadding: EdgeInsets.fromLTRB(10, 10, 10, 0),
                                  filled: selectedMistake != null ? false : true,
                                  fillColor: Colors.grey.shade300,
                                  hintText: "กรุณากรอกรายละเอียด",
                                  border: OutlineInputBorder(
                                      borderSide: BorderSide(color: Colors.grey.shade700)
                                ...
                                child: Container(
                                  ...
                                  child: ElevatedButton(
                                      ...
                                      onPressed: _header.text.isNotEmpty &&
                                                ...
                                                selectedMistake != null && _repeatsame.text.isNotEmpty
                                      ? ()=>setState(() {
