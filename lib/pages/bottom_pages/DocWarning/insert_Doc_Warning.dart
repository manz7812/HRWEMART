...
import 'Doc_Warning_Detail.dart';
import 'Doc_Warning_Edit.dart';

class InsertDocWarningPage extends StatefulWidget {
  const InsertDocWarningPage({Key? key}) : super(key: key);

  @override
  State<InsertDocWarningPage> createState() => _InsertDocWarningPageState();
}

class _InsertDocWarningPageState extends State<InsertDocWarningPage> {
  ...
  Future<Null> getlistPos() async{
    String UrlPos = pathurl.listPosWarning;
  ...
  Future<Null> getdata() async {
    try{
      ...
      String url = pathurl.getDTDocWarning+"?status=$selectedStatus&year=$selectedYear&month=$selectedMonth";
  ...
  Future<Null> searchdata(year,month,status,posId) async{
    try{
      ...
      final String url = pathurl.getDTDocWarning+'?search=${empName.text}&pos_id=$posId&status=$status&year=$year&month=$month';
      final response = await get(
          Uri.parse(url),
          headers: {"Authorization": "Bearer $token"}
      );
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        title: const Text("ขอหนังสือเตือน"),
        // leading: IconButton(
        //   onPressed: () {
      ...
      body: loading ? LoaderOverlay(
        ...
        child: Column(
          children: [
            ButtonInsert().show_ButtonInsert_DocWaring(context,onGoBack),
            Expanded(
                ...
                child: SingleChildScrollView(
                  child: Column(
                    children: [
                      ...
                      dataDocWarning.isEmpty ? Nulldata() :
                      ListView.builder(
                        itemCount: dataDocWarning.length,
                        scrollDirection: Axis.vertical,
                        shrinkWrap: true,
                        physics: ScrollPhysics(),
                        itemBuilder: (context , i){
                          return getCard(dataDocWarning[i]);
                        },
                      ),
                    ],
                  ),
                      ...
                      child: TextButton(
                          ...
                          onPressed: (){
                            ...
                            Navigator.of(context).push(
                                PageTransition(
                                  child: DetailDocWarningPage(id: IDdoc,),
