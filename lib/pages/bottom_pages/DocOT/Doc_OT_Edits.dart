...
class EditsDocOTPage extends StatefulWidget {
  final String id;
  const EditsDocOTPage({Key? key, required this.id}) : super(key: key);

  @override
  State<EditsDocOTPage> createState() => _EditsDocOTPageState();
}

class _EditsDocOTPageState extends State<EditsDocOTPage> {
  bool loading = false;
    ...
    try{
      ...
      if (response.statusCode == 200) {
        setState(() {
          ...
          print("st.${startApi}");
          var liSstartApi = startApi.toString().split(" ");
          print("lisSt.${liSstartApi}");
          var lisTime = liSstartApi[1].split(":");
          print("lisTime.${lisTime}");
          var timeApi = "${lisTime[0]}:${lisTime[1]}";
          print("timeApi.${timeApi}");
          var startDate = "${liSstartApi[0]} ${timeApi}";
          startD = startDate;
          print("startD.${startD}");
        });
  ...
  Future<Null> listemp(date)async{
    print("${date}");
    String url = pathurl.listEmpOT+"?pos_id=${dataID[0]['emp_created']['pos_id']}&date=${date}&emp_type=$typeEmpName&event=$event&wc_id=$selectedKa";
    final response = await get(
        Uri.parse(url),
        headers: {"Authorization": "Bearer $token"}
    );
  ...
  Future<DateTime?> pickDate(BuildContext context) async {
    ...
    if (newdate == null){
      ...
      if(typeTimeName == "เต็มวัน"){
        listemp("${DateFormat('yyyy-MM-dd').format(initialDate)} ${timeStart}");
      }else{
      ...
      if(typeEmpName == "รายเดือน"){
        if(typeTimeName == "เต็มวัน"){
          listemp("${DateFormat('yyyy-MM-dd').format(newdate)} ${timeStart}");
        }else if(selectedOT == "3" && typeTimeName == "ชั่วโมง"){
          listemp("${DateFormat('yyyy-MM-dd').format(newdate)} ${timeStart}");
        }else{
          listemp("${DateFormat('yyyy-MM-dd').format(newdate)} ${timeEnd}");
        }
      }else{
        if(typeTimeName == "เต็มวัน"){
          listemp("${DateFormat('yyyy-MM-dd').format(newdate)} ${timeStart}");
  ...
  Future<Null> senddata() async{
    try{
      ...
      FormData formData = FormData.fromMap({
        ...
        "wc_id": selectedKa,
        "ot_date_start": dateOT == null ? sApi : DateFormat("dd/MM/yyyy HH:mm").parse(_startOTText.text),
        "ot_date_end": dateTimeOT == null ? eApi : DateFormat("dd/MM/yyyy HH:mm").parse(_endOTText.text),
  ...
  void checkTypeEmp(){
    if(dataID[0]['ot_emp_type'] == "รายเดือน"){
      typeEmp = 0;
      typeEmpName = "รายเดือน";
      listOT(typeEmp);
      selectedOT = dataID[0]['ot_type'].toString();
      print("typeEmp.${typeEmp}||typeEmpName.${typeEmpName}");
    }else{
      typeEmp = 1;
      typeEmpName = "รายวัน";
      listOT(typeEmp);
  ...
  void checkTypeTime(){
    if(dataID[0]['ot_time_format'] == "เต็มวัน"){
      typeTime = 0;
      typeTimeName = "เต็มวัน";
      print("typeTime.${typeTime}||typeTimeName.${typeTimeName}");
    }else{
      typeTime = 1;
      typeTimeName = "ชั่วโมง";
      print("typeTime.${typeTime}||typeTimeName.${typeTimeName}");
  ...
  Widget build(BuildContext context) {
    return Scaffold(
        ...
        body: loading && typeEmp != null && typeEmpName != null && selectedOT != null ?
        LoaderOverlay(
      ...
      case 0: ////รายเดือน
        return Column(
          children: [
                ...
                TextFormField(
                  ...
                  onTap: typeTimeName == "เต็มวัน"
                    ...
                    if(clerTxT == null){
                      ...
                      if(dataID[0]['ot_total_hours'] > double.parse(hourOT.text) || dataID[0]['ot_total_hours'] < double.parse(hourOT.text)){
                        listemp(startD);
                      }
                    }else{
                      clertxt();
            ...
            hourOT.text.isNotEmpty && selectedPO.isNotEmpty && textEditingControllers.isNotEmpty && numMoney.isNotEmpty
            ? empUI()
            : const SizedBox(),
            const SizedBox(
              height: 20,
