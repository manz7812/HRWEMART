...
class EditDocGoodMemoryPage extends StatefulWidget {
  final String id;
  const EditDocGoodMemoryPage({Key? key, required this.id}) : super(key: key);

  @override
  State<EditDocGoodMemoryPage> createState() => _EditDocGoodMemoryPageState();
}

class _EditDocGoodMemoryPageState extends State<EditDocGoodMemoryPage> {
  bool loading = false;
  ...
  Future<Null> listemp()async{
    String url = pathurl.listempScore+"?pos_id=${dataID[0]['emp_created']['pos_id']}";
    final response = await get(
        Uri.parse(url),
        headers: {"Authorization": "Bearer $token"}
    );
  ...
  Future<Null> senddata(data) async{
    String url = pathurl.updateDTDocScoreID+widget.id;
  ...
  void initState() {
    ...
    Future.delayed(const Duration(seconds: 1),(){
      ...
      _detial.text = dataID[0]['ss_name'];
      selectedGoodOrBad = dataID[0]['ss_title'];
      scoretxt.text = dataID[0]['ss_score'].toString();
      score = 0;
      // scoretxt.text = score.toString();

      selectedPO[0]=dataID[0]['ss_emp']['id'];
      print("selectedPO = $selectedPO");
      ListempID.add(dataID[0]['ss_emp']['id']);
      getscoreEmp(dataID[0]['ss_emp']['id']);
      print("ListempID = $ListempID");
      textEditingControllers[0].text = "${dataID[0]['ss_emp']['name'].split(" ")[1]} ${dataID[0]['ss_emp']['name'].split(" ")[2]}";
      wlPos = dataID[0]['ss_emp']['pos_id'];
      print("wlPos = $wlPos");
  ...
  Widget build(BuildContext context) {
    return Scaffold(
        ...
        appBar: AppBar(
          title: const Text("แก้ไขบันทึกความดี"),
          centerTitle: true,
          elevation: 0,
