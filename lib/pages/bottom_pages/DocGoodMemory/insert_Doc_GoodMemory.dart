...
import 'Doc_GoodMemory_Detail.dart';

class InsertDocGoodMemoryPage extends StatefulWidget {
  const InsertDocGoodMemoryPage({Key? key}) : super(key: key);

  @override
  State<InsertDocGoodMemoryPage> createState() => _InsertDocGoodMemoryPageState();
}

class _InsertDocGoodMemoryPageState extends State<InsertDocGoodMemoryPage> {
  bool loading = false;
  ...
  Future<Null> getlistPos() async {
    String UrlPos = pathurl.listPosScore;
  ...
  Future<Null> getdata() async {
    try{
      ...
      String url = pathurl.getDTDocScore+"?status=$selectedStatus&year=$selectedYear&month=$selectedMonth";
  ...
  Future<Null> searchdata(year,month,status,posId) async{
    try{
      ...
      final String url = pathurl.getDTDocScore+'?search=${empName.text}&pos_id=$posId&status=$status&year=$year&month=$month';
      final response = await get(
          Uri.parse(url),
          headers: {"Authorization": "Bearer $token"}
      );
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: loading ? LoaderOverlay(
        ...
        child: Column(
          children: [
            ButtonInsert().show_ButtonInsert_DocGoodMemory(context,onGoBack),
            Expanded(
    ...
    return InkWell(
      onTap: (){
        Navigator.of(context).push(
            PageTransition(
              child: DetailDocGoodMemoryPage(id: IDdoc,),
