...
import 'Doc_GoodMemory_Edit.dart';

class DetailDocGoodMemoryPage extends StatefulWidget {
  final String id;
  const DetailDocGoodMemoryPage({Key? key, required this.id}) : super(key: key);

  @override
  State<DetailDocGoodMemoryPage> createState() => _DetailDocGoodMemoryPageState();
}

class _DetailDocGoodMemoryPageState extends State<DetailDocGoodMemoryPage> {
  bool loading = false;
  ...
  Future<Null> getdata() async {
    try{
      ...
      String url = pathurl.getDTDocScoreID+widget.id;
      ...
      if (response.statusCode == 200) {
        setState(() {
          ...
          dataEmp.add(dataID[0]['ss_emp']);
          if(dataID[0]['bounce'] == ""){
            dataNote = [];
          }else{
            dataNote = dataID[0]['bounce'];
          }
  ...
  Future<Null> setStatusNO(id,status,remark) async{ /*ยังไม่แก้ */
    try{
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        title: const Text("รายละเอียดบันทึกความดี"),
        // leading: IconButton(
        //   onPressed: () {
            ...
            child: Column(
              children: [
                Card(
                  ...
                  child: Container(
                    width: double.infinity,
                    decoration: dataID[0]['ss_status'] == 'ยกเลิก' ? BoxDecoration(
                          ...
                          child: Row(
                            ...
                            children: [
                              ...
                              Text(
                                "${dataID[0]['ss_status']}",
                                style: TextStyle(
                                  color: dataID[0]['ss_status'] == 'อนุมัติ' ? Colors.green
                                      : dataID[0]['ss_status'] == 'ไม่อนุมัติ' || dataID[0]['ss_status'] == 'ยกเลิก'? Colors.red
                                      : dataID[0]['ss_status'] == 'ตีกลับ' ? Colors.orange
                                      : Colors.black87,
                                  fontWeight: FontWeight.w400,
                                  fontSize: 16,
                                ),
                                ...
                                child: TextButton(
                                    ...
                                    onPressed: (){
                                      Navigator.of(context).push(
                                          PageTransition(
                                            child: EditDocGoodMemoryPage(id: widget.id,),
  ...
  Widget ListEmp(item){
    var posName = item['pos_name'];
    var imgurl = item['img_url'].toString();
    var Name = item['name'].toString().split(' ');
        ...
        Row(
          children: [
            ...
            Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                Text("${posName}",style: TextStyle(fontSize: 16,fontWeight: FontWeight.w600,color: Colors.black87),),
                Text("${Name[1]} ${Name[2]}(${nicname})",style: TextStyle(fontSize: 16,fontWeight: FontWeight.w500,color: Colors.black87),),
              ],
            ),
          ],
        ),
